defmodule FleatureWeb.UserConfirmationControllerTest do
  ...
  describe "POST /users/confirm" do
    ...
    test "sends a new confirmation token", %{conn: conn, user: user} do
      ...
      assert Repo.get_by!(Schemas.UserToken, user_id: user.id).context == "confirm"
    end

    test "does not send confirmation token if User is confirmed", %{conn: conn, user: user} do
      Repo.update!(Schemas.User.confirm_changeset(user))
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      refute Repo.get_by(Schemas.UserToken, user_id: user.id)
    end
    ...
    test "does not send confirmation token if email is invalid", %{conn: conn} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      assert Repo.all(Schemas.UserToken) == []
    end
  end
